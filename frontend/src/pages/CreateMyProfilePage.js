...
import { updateSelfActorInfo } from '../api/actorApi';
...
const CreateMyProfilePage = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const data = await updateSelfActorInfo(actorData);
      
      if (!data || !data.id) {
        throw new Error('创建演员资料失败，未返回有效数据');
      }
      
      message.success('演员资料创建成功！', 1.5, () => {
        // 创建成功后跳转到上传媒体资料页面
        navigate(`/actors/${data.id}/upload-media`);
      });
    } catch (error) {
      console.error('创建个人资料失败:', error);
      message.error('创建个人资料失败: ' + (error.response?.data?.detail || error.message || '未知错误'));
      setLoading(false);
    }
  };
    ...
    <Spin spinning={loading}>
      <Card title="创建我的演员资料">
        <Steps current={['basic', 'professional', 'contact'].indexOf(activeTab)} style={{ marginBottom: 30 }}>
          <Step title="基本信息" description="填写基本资料" status={allTabsVisited.basic ? 'finish' : 'wait'} />
...
export default CreateMyProfilePage; 